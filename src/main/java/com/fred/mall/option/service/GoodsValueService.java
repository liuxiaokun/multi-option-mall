...
public interface GoodsValueService {
     ...
     * 保存一条 GoodsValue 数据。
     *
     * @param goodsValue 待保存的数据。
     * @throws BizException 保存失败异常。
     */
    void saveGoodsValue(GoodsValue goodsValue) throws BizException;

    /**
     * 保存多条 GoodsValue 数据。
     *
     * @param goodsValueList 待保存的数据列表。
     * @throws BizException 保存失败异常。
     */
    void saveGoodsValueList(List<GoodsValue> goodsValueList) throws BizException;

    /**
     * 根据id更新 GoodsValue，字段为null的选项会把数据库字段更新为null，即全部更新。
     *
     * @param goodsValue 更新的目标数据。
     */
    void updateGoodsValue(GoodsValue goodsValue);

    /**
     * 根据Id部分更新实体 GoodsValue。
     *
     * @param dataMap 需要更新的键值对。
     * @param conditionMap where语句后的条件筛选的键值对。
     */
    void updateGoodsValueSelective(Map<String, Object> dataMap, Map<String, Object> conditionMap);

    /**
     * 根据id逻辑删除一条 GoodsValue。
     *
     * @param id 数据id。
     * @param userId 删除人的id。
     * @throws BizException 逻辑删除异常。
     */
    void logicDeleteGoodsValue(Long id, Long userId) throws BizException;

    /**
     * 根据id物理删除一条 GoodsValue。
     *
     * @param id 数据唯一id。
     * @throws BizException 物理删除异常。
     */
    void deleteGoodsValue(Long id) throws BizException;

    /**
     * 根据id查询一条 GoodsValue。
     *
     * @param id 数据唯一id。
     * @return 查询到的 GoodsValue 数据。
     */
    GoodsValueDTO findGoodsValueById(Long id);

    /**
     * 根据条件查询得到第一条 GoodsValue。
     *
     * @param params 查询条件
     * @return 符合条件的一个 GoodsValue。
     */
    GoodsValueDTO findOneGoodsValue(Map<String, Object> params);
     ...
     * @return 查询结果的数据集合。
     */
    List<GoodsValueDTO> find(Map<String, Object> params, Vector<SortingContext> scs, PagingContext pc);
